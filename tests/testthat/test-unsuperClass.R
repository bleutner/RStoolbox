...
test_that("unsuperClass output modes",{
    for(cm in c(TRUE, FALSE)) {    
        expect_is(unsuperClass(lsat, clusterMap = cm, nClasses = 2, output = "classes")$map, "RasterLayer")
        expect_is(scm <- unsuperClass(lsat, clusterMap = cm,  nClasses = 2, output = "distances")$map, "RasterBrick")
        expect_equal(nlayers(scm), 2)
    }
})
