...
test_that("fit mlc",{
          expect_is( mr <- RStoolbox:::mlc(mat,y), "list")
          expect_equal(names(mr), c("a", "b", "levels")) 
          expect_equal(vapply(mr$a, length, numeric(1)), c(m=3,D=1,I=9))   
      })

test_that("predict mlc",{
                      mod <- train( mat, y, method = RStoolbox:::mlcCaret, trControl = trainControl(method = "none"))
                      expect_is(pred <- RStoolbox:::predict.mlc(mod, mat), "factor")
                      expect_equal(length(pred), nrow(mat))
                      expect_equal(levels(pred), c("a", "b"))
                      expect_is(prob <- RStoolbox:::predict.mlc.prob(mod, mat), "matrix")
