...
for (lib in c("raster", "terra")) {
  ...
  test_that("errors and warnings are emitted", {
    ...
    expect_is(spectralIndices(r, red = 1, nir = 2, blue = 1, index = c( "EVI"), skipRefCheck = TRUE), "SpatRaster")
  ...
  test_that("maskLayer is considered regardless of input class",  {
    for(i in 1:3){
      expect_s4_class(nd <- spectralIndices(.stack(r,ml), red = 1, nir=2, indices = "NDVI",
                                            maskLayer = m_cfg[[i]], maskValue = 10), "SpatRaster")
      expect_equal(nd[,2], nadf)
      expect_s4_class(nd <- spectralIndices(.stack(r,ml), red = 1, nir=2, indices = "NDVI", 
                                            maskLayer = m_cfg[[i]], maskValue = NA), "SpatRaster")
