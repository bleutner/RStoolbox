...
test_that("sf inputs",{
			expect_is( superClass(img, trainData = st_as_sf(train), nSamples = 50, responseCol = "class", model = "pls", 
							tuneGrid = data.frame(ncomp = 3), tuneLength = 1, trainPartition = 0.7, predict = FALSE), "superClass")
			
		})

test_that("terra inputs",{
			expect_is( superClass(rast(img), trainData = train, nSamples = 50, responseCol = "class", model = "pls", 
							tuneGrid = data.frame(ncomp = 3), tuneLength = 1, trainPartition = 0.7, predict = FALSE), "superClass")
