...
#' @export
superClass <- function(inputRaster, trainingData, validationData = NULL, responseCol = NULL, nSamples = 100,
        areaWeightedSampling = TRUE, polygonBasedCV = FALSE, 
        model = "rf", tuneLength = 3,  kfold = 10,
        filename = NULL, verbose = FALSE,
        predict = TRUE, overwrite = TRUE, ...) {
    # TODO: add examples
    # TODO: check applicability of raster:::.intersectExtent 
    # TODO: check responseCol for testData    
    ...
    if(!is.null(validationData) && !responseCol %in% colnames(validationData@data)) 
        stop(paste0("The column ", responseCol, " does not exist in validationData. \nAvailable columns are: ", colnames(validationData@data,collapse=", ")), call. = FALSE) 
    ...
    if(polygonBasedCV){
        folds <- createFolds(trainingData@data[[responseCol]], k = kfold)
        names(folds) <- NULL
        folds <- melt(folds) 
        foldCol <- "excludeFromFold"
        trainingData@data[[foldCol]]<- folds[order(folds$value),"L1"]
    } else {
        foldCol <- NULL
    }
    ...
    .samplePixels <- function(SHAPE, RASTER, foldCol = NULL){
        ...
        dataSet <- data.frame(
                if(classType == "polygons") over(x = xy, y = SHAPE)[c(responseCol,foldCol)] else SHAPE[[responseCol]],
    ...
    dataSet  <- .samplePixels(SHAPE = trainingData, RASTER=inputRaster, foldCol=foldCol)
    if(polygonBasedCV) {
    ...
    model 	<- train(response ~ ., data = dataSet, method = model, tuneLength = tuneLength, 
            trControl = trainControl(method = "cv",index = indexIn), ...)   
    ...
    modelFit <- getTrainPerf(model)
    dataType <- NULL
    if(mode == "classification") {
        dataType <- if(length(classes) < 255) "INT1U" else "INT2U"
        modelFit <- list(modelFit, confusionMatrix(model, norm = "average"))     
    } 
    ...
    ## VALIDATION ########################
    valiSet <- .samplePixels(validationData, spatPred)
    colnames(valiSet) <- c("reference", "prediction")
    if(mode == "classification"){
        if(!is.factor(valiSet$reference)) trueClass <- factor(trueClass, levels = levels(classes))
    ...
    out <- list(model = model, modelFit = modelFit, validation = validation, map = spatPred)
